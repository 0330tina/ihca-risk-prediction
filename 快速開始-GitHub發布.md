...
## ⚙️ 第四步半：設定 Git 用戶資訊（如果還沒設定過）

**如果這是您第一次使用 Git，需要先設定身份：**

```bash
# 設定您的姓名（替換成您的真實姓名）
git config --global user.name "您的姓名"

# 設定您的電子郵件（建議使用 GitHub 電子郵件）
...
**或使用您的 GitHub 電子郵件：**
```bash
git config --global user.email "您的GitHub電子郵件@gmail.com"
```
...
1. **直接點擊這個連結**：https://github.com/settings/tokens/new
   - 如果已登入，會直接進入建立 Token 的頁面

2. **填寫資訊**：
   - **Note**: 輸入 `Git Push`（隨便取個名字）
   - **Expiration**: 選擇 `90 days`（建議）
   - **勾選 `repo` 權限**（這是最重要的！）

3. **點擊綠色的「Generate token」按鈕**

4. **⚠️ 立即複製 Token！**（只會顯示一次）
   - 建議先複製到記事本保存

5. **回到終端機，貼上這個 token 作為密碼**
...
1. **GitHub 首頁** → 點擊右上角**頭像**
2. 選擇 **「Settings」**
3. 左側選單**向下滾動到最底部**
4. 找到 **「Developer settings」**（在最下面）
5. 點擊 **「Personal access tokens」** → **「Tokens (classic)」**
6. 點擊 **「Generate new token (classic)」**
7. 填寫資訊並生成 Token

**如果還是找不到，直接使用連結：** https://github.com/settings/tokens/new
